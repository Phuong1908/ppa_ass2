...
% built-in functions
type_check_stmt(Env,call(writeInt,[X])) :- 
    get_type_expression(Env,X,integer).
type_check_stmt(Env,call(writeIntLn,[X])) :- 
    get_type_expression(Env,X,integer).
type_check_stmt(Env,call(writeReal,[X])) :- 
    get_type_expression(Env,X,float).
type_check_stmt(Env,call(writeRealLn,[X])) :- 
    get_type_expression(Env,X,float).
type_check_stmt(Env,call(writeBool,[X])) :- 
    get_type_expression(Env,X,boolean).
type_check_stmt(Env,call(writeBoolLn,[X])) :- 
    get_type_expression(Env,X,boolean).
type_check_stmt(Env,call(writeStrLn,[X])) :- 
...
%reduce_all: reduce all expressions in the list until there is no expression to reduce
